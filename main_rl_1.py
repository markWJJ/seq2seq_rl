...
import numpy as np
from jd_data.Data_deal import DataDealSeq
...
class Seq2SeqRl(object):
    ...
    def __build_model__(self):
        ...
        print(action_batch_one)
        action_probs=tf.reduce_mean(tf.multiply(self.soft_logit,action_batch_one),2)
        print(action_probs)
...
class ENV(object):
    ...
    def __init__(self):
        self.dd = DataDealSeq(train_path="/write.txt", test_path="/test.txt",
                         dev_path="/test.txt",
    ...
    def step(self,action,state,step,max_turn):
        ...
        #     reward = bleu_score
        num=0
        for e,e1 in zip(true_dec,this_dec):
            if e==e1:
                num+=1
        reward=float(num)/float(len(true_dec))
        if reward==0.0:
            reward=-2
        elif reward<0.5:
            reward=1
...
def deep_q_learning(sess,
    ...
    for i in range(replay_memory_init_size):
        ...
        for step in range(1,2*FLAGS.decoder_len):
            action = policy(sess=sess, observation=state, epsilon=epsilons[min(0, epsilon_decay_steps-1)],step=step)
    ...
    for i_episode in range(200):
        ...
        for t in range(1,2*FLAGS.decoder_len):
            ...
            # Take a step
            action = policy(sess, state, epsilon,step)
            # action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
